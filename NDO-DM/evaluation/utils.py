...
def prep_sklearn_models():
    ...
    model_specs['linear_svc'] = {
        'max_iter': 20000, 'tol': 1e-8, 'loss': 'hinge'}
...
def compute_precision_recall(real_features, fake_features, nhood_size=3, eps=1e-6):
    
    real_features = real_features.astype(np.float32)
    fake_features = fake_features.astype(np.float32)

    
    real_nn = NearestNeighbors(n_neighbors=nhood_size).fit(real_features)
    fake_nn = NearestNeighbors(n_neighbors=nhood_size).fit(fake_features)
    ...
    fake_radius = fake_distances[:, -1]

    # 
    dists = real_nn.kneighbors(fake_features, return_distance=True)[0][:, 0]
    precision = np.mean(dists < np.mean(real_radius) + eps)

    # 
    dists = fake_nn.kneighbors(real_features, return_distance=True)[0][:, 0]
    recall = np.mean(dists < np.mean(fake_radius) + eps)
